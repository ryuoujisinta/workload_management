...
- **プロジェクト管理**: プロジェクトの登録、名称編集、および年度ごとの管理。
- **タスク管理**: プロジェクトに紐づくタスクの登録、名称編集、および月ごとの有効化。CSVによる一括登録にも対応（後述）。
...
「タスク管理」画面から、登録済みのプロジェクト名とタスク名を編集できます。

1. **プロジェクト名**: 「プロジェクト管理」の対象プロジェクト名を変更し、「保存」を選択します。
2. **タスク名**: タスク一覧の対象タスク名を変更し、「保存」を選択します。

名称の前後にある空白は保存時に除去されます。空の名称や、既に使用されているプロジェクト名は保存できません。
